...
export const workflow = {
  ...
  onStateChanged: (callback: (payload: WorkflowStateChanged) => void): Unsubscribe => {
    const handler = (_: unknown, payload: WorkflowStateChanged) => callback(payload);
  ...
  onTaskCompleted: (
    ...
    const handler = (_: unknown, payload: WorkflowTaskCompleted) => callback(payload);
    ipcRenderer.on("workflow:taskCompleted", handler);
    return () => ipcRenderer.removeListener("workflow:taskCompleted", handler);
  },
  onTaskFailed: (callback: (payload: WorkflowTaskFailed) => void): Unsubscribe => {
    const handler = (_: unknown, payload: WorkflowTaskFailed) => callback(payload);
    ipcRenderer.on("workflow:taskFailed", handler);
    return () => ipcRenderer.removeListener("workflow:taskFailed", handler);
  },
  onCompleted: (callback: (payload: WorkflowCompleted) => void): Unsubscribe => {
    const handler = (_: unknown, payload: WorkflowCompleted) => callback(payload);
  ...
  onAiPartial: (callback: (payload: WorkflowAiPartial) => void): Unsubscribe => {
    const handler = (_: unknown, payload: WorkflowAiPartial) => callback(payload);
