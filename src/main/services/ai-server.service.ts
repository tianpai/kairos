...
class AIServerService {
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/api/ai/execute', async (req, res) => {
      const { id, taskType, payload, provider, apiKey, model } = req.body as ExecuteRequest

      try {
        const keyPreview = apiKey ? `${apiKey.substring(0, 10)}...` : 'MISSING'
        log.info(`[AIServer] HTTP task started: ${taskType} with ${provider}/${model} (key: ${keyPreview})`)
        const startTime = Date.now()

        const result = await this.executeTask(taskType, payload, { type: provider, apiKey }, model)
    ...
    this.wss.on('connection', (ws) => {
      ...
      ws.on('message', async (data) => {
        try {
          ...
          log.info(`[AIServer] WS task started: ${taskType} with ${provider}/${model}`)
          const startTime = Date.now()
          ...
          try {
            ...
            const duration = ((Date.now() - startTime) / 1000).toFixed(2)
            log.info(`[AIServer] WS task completed: ${taskType} (${duration}s)`)
            ...
            log.error(`[AIServer] WS task failed: ${taskType} - ${errorMessage}`)

            if (ws.readyState === WebSocket.OPEN) {
              ws.send(JSON.stringify({ id, type: 'failed', error: errorMessage }))
            }
          }
        } catch (parseError) {
          log.error('[AIServer] Failed to parse WebSocket message:', parseError)
        }
      })
    ...
    return new Promise((resolve, reject) => {
      this.server.listen(0, '127.0.0.1', () => {
        ...
        if (address && typeof address === 'object') {
          ...
          log.info(`[AIServer] Express server started on http://127.0.0.1:${this.port}`)
          log.info(`[AIServer] WebSocket available at ws://127.0.0.1:${this.port}/ws/ai`)
          resolve(this.port)
        } else {
